...
//include ('include/Pagination.Class.php'); // For pagination
 ...
 $order_by_asc_desc    = $_REQUEST['order_by'];
  ...
  if($_REQUEST['serch_item_value']==1)
  {
  $objItem->status   = 1 ;
  $smarty->assign("serch_item_value_chk",1);
   // for  active  items
  }
  if($_REQUEST['serch_item_value']==3)
  {
  // for  hold  items 
  $objItem->status   = 3 ;
  $smarty->assign("serch_item_value_chk",3);
  }
  if($_REQUEST['serch_item_value']==0)
  {
  $smarty->assign("serch_item_value_chk",0);
  $objItem->status   ='';
  }
...
if($make_item_haat!='')
{
    $objCoupon->item_id           = $make_item_haat;
    $objCoupon->coupon_status     = 0;
    $objCoupon->insertUpdateCouponStatus();
          
    $objItem->item_value          = $make_item_haat;
    $objItem->hatting_status      = 1;
    $image_details_item1          = $objItem->insertUpdateItem1('');
    success_msg("Item has been been send to haating list !!");
    redirect('my-haating-items-list.php');
    //insertUpdateCouponStatus();
}
...
if($remove_hold_item_id!='')
{
     ...
     $objItem->item_value    = $remove_hold_item_id;
     $image_details_item1    = $objItem->insertUpdateItem1('1');
...
*/}
  ...
  // $objItem->locker_status  = 0;    // bcoz items send in locker cannot be sent into haating process
