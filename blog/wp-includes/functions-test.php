...
function wp_upload_dir( $time = null ) {
	...
	//echo 'ris=='.$rishi_path = '\home\nethaatadmin\public_html\blog\wp-content\uploads\\'.$subdir;
	//echo 'ris=='.str_replace('/','\\',$uploads['path'] );
	if ( ! wp_mkdir_p( str_replace('/','\\',$uploads['path'] )) ) { echo 'laal--'.$uploads['path'];exit;
...
function wp_nonce_ays( $action ) {
	...
	//wp_die( $html, $title); //commented by mahipal
	/**
	*@Created By : Mahipal Adhikari
	*@Created On : 10-Dec-09
	*@Description: redirect user to admin login page once looged out from wordpress blog
	**/
