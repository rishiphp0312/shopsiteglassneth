<?php
mysql_connect("localhost",'root','')or die('error in db') ;
...
if($post_content!=''){
	...
	$post_content = str_replace("[gallery]",'',$post_content);
	$post_content = str_replace("[/gallery]",'',$post_content);
	$string = explode('</a>', $post_content);*/
	$post_contents = trim(strip_tags($post_content));
	$post_contents = preg_replace( '|\[(.+?)\](.+?\[/\\1\])?|s', '',$post_contents );
	$returnRecentPost.='<div style="height:30px;" class="imgTextDesc">'.substr($post_contents,0,75).'</div><br clear="all">';
