...
{
	var aa= document.getElementById('msg_trash');
	if (checked == false)
	{
		checked = true
	}
		    ...
		    <table width="100%" align="center" cellpadding="0" cellspacing="0" style="border:1px solid #8A5F40;padding:5px;">
			  <tr>
				...
				<td valign="top" width="83%">				  
				  <div id="mailbox_1">
				  <form action="" method="get" name="msg_trash" id="msg_trash">
				  <table width="100%" align="right" cellpadding="4" cellspacing="1" bgcolor="#FFFFFF">
				    <tr>
					  <td colspan="4">
					    <div style="padding:3px; background:#eeeeee; margin-bottom:5px;">
						  ...
						  <input name="search_message" class="Class_Button_ris" 
						  type="submit" value="Search" 
						  style="cursor:pointer;width:60px;" />
						  </div>
						   <div style="width:370px; float:right; text-align:right;">
						  <span style="font-size:10px;">Sort By </span>
						  <select name="sorting" style="padding:1px; font-size:10px;" onchange="document.msg_trash.submit();">
						  ...
						  <select name="order" style="padding:1px; font-size:10px;" onchange="document.msg_trash.submit();">
						  ...
						  <select name="limit" style="padding:1px; font-size:10px;" onchange="document.msg_trash.submit();">
					  ...
					  <input name="chk_delete_trash" type="submit" value="Delete" class="Class_Button_ris" /> 
					  <input name="chk_restore" type="submit" value="Restore" class="Class_Button_ris"  /> 
					  </td>
					</tr>

					<tr bgcolor="#e3e6d1">
					  <td width="22"><input name="main_check" type="checkbox" onclick="checkedAll(msg_trash);" /></td>
					  <td width="120"><strong class="blue_clr">Sender</strong></td>
					  <td width="210"><strong class="blue_clr">Subject</strong></td>
					  <td width="90"><strong class="blue_clr">Date</strong></td>
					</tr>
					{if $num_trash>0}
					{section name=val loop=$trash_msg}
					<tr bgcolor="#e5f3f9">
						<td>
							<input name="checked_msg[]" id="checked_msg[]" value="{$trash_msg[val].subid}" type="checkbox" />
						</td>
						{if $trash_msg[val].inbox_read==1}
							<td style="font-weight:bold;">		{$trash_msg[val].username}
							</td>
						{else}
							<td >		{$trash_msg[val].username}
							</td>
						{/if}
						{if $trash_msg[val].inbox_read==1}
						<td style="font-weight:bold;">
						
							<a href="view_messages.php?msg_id={$trash_msg[val].subid}" class="blue_link">{$trash_msg[val].subject}</a>
						</td>
						{else}
						<td >		
							<a href="view_messages.php?msg_id={$trash_msg[val].subid}" class="blue_link">{$trash_msg[val].subject}</a>
					...
					<tr>
					  <td colspan="4" align='center' style='text-align:center;color:#7e354d;font-family:arial;'>
					 No records found !!
					  </td>
					  </td>
					</tr>
