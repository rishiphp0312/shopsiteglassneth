...
<img src="images/cat_hdbull.jpg" alt="" class="vAlign" /> Featured Items</div>
				...
				<div class="grpImgBotBdr">
					...
					<div class="imgText">
					<a href="item-details.php?details_item_value={$v.item_id}" title="{$v.title|ucfirst}"><strong>
  ...
  </span>{/if}{literal}<script language="javascript" >
  <code class="mix">$('#set1*').tooltip();</code>
		...
		<div class="imgText">
		<a href="item-details.php?details_item_value={$v_r.item_id}" title="{$v_r.title|ucfirst}"><strong>
		  ...
		  {/if}
			...
			<div class="imgCatSec">
				...
				<div class="grpImgBotBdr">
					...
					<div class="imgLftBdr">
						<div><a href="http://{$add_this_name_www}.{if $smarty.server.HTTP_HOST=='www.nethaat.com'}{$store.username}.{/if}{$add_this_name}/featured_store_information.php" title="{$store.username|ucfirst}"><img src="{$baseUrl}getthumb.php?w=150&h=100&fromfile=uploads/{$store.v_store_image}" onerror="this.src='{$baseUrl}images/item_small_img.jpg'; this.style.width='150px'; this.style.height='100px';" alt="{$store.username|ucfirst}" class="catImgBdr" /></a></div>
