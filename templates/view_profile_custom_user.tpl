...
<span class="mainHD">Request Details</span>
	...
	<div class="myitemmain">
		...
		<a href="advance_aggrement.php?itemid={$id}&buyerid={$buyid|base64_encode}">
...
<img src="{$baseUrl}getthumb.php?w=150&h=100&fromfile={if $citem[cus].image1!=''}uploads/{$citem[cus].image1}{else}images/no_img.jpg{/if}" alt="" border="0" class="buyerimg"  />			</td>
		</tr>
		...
		<tr>
		  ...
		  <td style="font-size:12px;">
<a href="download3.php?cust_id1={$citem[cus].id|base64_encode}">Nethaat-{$citem[cus].file_attached|basename}</a>
					</td>
		  </tr>
		  {/if}
		</table>
		...
		<td  colspan="2" style="padding-top:10px; ">
			...
			<tr>
				...
				<a href="download3.php?cust_id={$msg[custom].id|base64_encode}">Nethaat-{$msg[custom].file_attached|basename}</a>
				{/if}
				</td>
		...
		<form action="view_profile_custom_user.php" enctype='multipart/form-data' method="post" name="frmmessage">
			...
			<tr>
				...
				<input type='hidden' value='{$buyid|base64_encode}' name='buyid' >
				</td>
			</tr>
			...
			<tr>
				<td style="">{section name=cus loop=$citem}
					...
					<input type="hidden" name="id" value="{$citem[cus].id|base64_encode}">
					<input type="hidden" name="buyid" value="{$citem[cus].user_id|base64_encode}">
