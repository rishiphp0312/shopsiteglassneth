...
if(val==1)
	{
	jConfirm('Do you really want to delete this record?', 'Confirm', function(r)
	{
		if(r)
		{
			location.href='admin_view_seo.php?delete_item_value='+id;
		}
		else
		{
			return false;
...
<tr>
	  <td align="right" valign="top"  ><a href='admin_page_seo.php'>Add New</a></td></tr>
	<tr>
      ...
      <table width="100%" cellpadding="4" cellspacing="2" align="center">
       {if $productList && $no_id_fetched!='0'}
		...
		<tr bgcolor="{cycle values='#f5f5f5,#e6e6e6'}">
		  ...
		  {/if}		  </td>
                  <td>{$prod.page_name}</td>
                  <td>{$prod.meta_title|ucfirst}</td>
		  <td><a href="admin_page_seo.php?meta_id={$prod.meta_id}"><img src="{$baseUrl}/images/edit_icon.png" alt="Edit" border="0" /></a>&nbsp;
		  <a href="javascript: void(0);" title="Delete" style="text-decoration:none;"   onclick="confirm_msg({$prod.meta_id},{1});">
