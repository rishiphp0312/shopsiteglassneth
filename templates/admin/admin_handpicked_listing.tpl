...
function confirm_msg(id,val,URL)
{
//alert(URL+'==URL');
...
if(val==1)
{
    jConfirm('Do you really want to Suspend this Item?', 'Confirm', function(r) 
	{
		if(r)
		{
			location.href='admin_handpicked_listing.php?suspend_item_value='+id+'&'+URL;
		}
		else
		{
			return false;
 ...
 jConfirm('Do you really want to Approve this Item?', 'Confirm', function(r) 
	{
		if(r)
		{
			location.href='admin_handpicked_listing.php?approve_item_value='+id+'&'+URL;
		}
		else
		{
			return false;
...
{
 jConfirm('Do you really want to delete this Item from  Featured List?', 'Confirm', function(r)
	{
		if(r)
		{
			location.href='admin_handpicked_listing.php?soft_handpicked_item_value='+id+'&'+URL;
		}
		else
		{
			return false;
	...
	jConfirm('Do you really want to delete this Item?', 'Confirm', function(r) 
	{
		if(r)
		{
			location.href='admin_handpicked_listing.php?delete_item_value='+id+'&'+URL;
		}
		else
		{
			return false;
...
<input name="inventory_alert" 	type="text" id="inventory_alert" class="input"
 value="{$inventory_alert}" /></td>
			
			<td colspan='2'  bgcolor="#FFFFFF"  nowrap>
			Date Modified<br><select style='font-size:12px;width:70px;'  name="msel_days">
                 <option value='0' >--Days--</option>
		 {html_options values=$no_of_days_curentmonth output=$no_of_days_curentmonth selected=$msel_days}
			</select>&nbsp;&nbsp;<select name="msel_month">
                       <option value='0' >--Month--</option>
		       {html_options values=$array_for_id output=$array_for_month selected=$msel_month}
			</select> &nbsp;&nbsp;
		    <select name="msel_year">{html_options values=$year_12 output=$year_12 selected=$msel_year }
			</select>&nbsp;


			<!--Status <input type="radio" name="status" value="1" {$chkActive}> Approved
			...
			value="Search" />
			&nbsp;&nbsp;<br><br> &nbsp;  <input type="button" class="button" name="reset"
			value="Clear Search" onclick="window.location='admin_handpicked_listing.php';" />
			</td>
		  </tr>
      ...
      <table width="100%" cellpadding="4" cellspacing="2" align="center">
       {if $productList && $no_id_fetched!='0'}
		...
		<tr bgcolor="{cycle values='#f5f5f5,#e6e6e6'}">
		  ...
		  <td>{'$ '|cat:$prod.cost_item}</td> <td>{$prod.username}</td>
		 ...
		 &nbsp;|&nbsp;<a href="javascript: void(0);" title="Approve It" style="text-decoration:none;"onclick="confirm_msg({$prod.item_id},{2});">Approve</a>
		  ...
		  onclick="confirm_msg({$prod.item_id},{1},'{$pass_url_infun}');">Suspend</a>
                {elseif $prod.status==2}
                <a href="javascript: void(0);" title="Approve It" 
		style="text-decoration:none;" 
		 onclick="confirm_msg({$prod.item_id},{2},'{$pass_url_infun}');">Approve</a>

 {/if}
		  </td>
		  <td nowrap>
			  <a href="admin_edit_product.php?item_id={$prod.item_id}&user_type={$prod.user_type}&ftype=1" 
			  style="text-decoration:none;" title="Edit">
			   ...
			   &nbsp; &nbsp;
			  {if $prod.make_handpicked==1} 
		 <a href="javascript: void(0);" title="Delete from  Featured" style="text-decoration:none;"   onclick="confirm_msg({$prod.item_id},{6},'{$pass_url_infun}');"><img src="{$baseUrl}/images/dlt_icon.png" alt="Delete from  Featured"   border="0" /></a>
			  {/if}
