...
class CRYPTO 
{
  ...
  function CRYPTO($key = 'NethaatActivationKey', $iv = false, $algorithm = 'tripledes', $mode = 'ecb') 
  { 
...
function getKey($filename=MYKEY)
{
	$fp=fopen($filename,'r');
	$key = fread ($fp, filesize ($filename)); 
	return $key;
}
function decryptStr($str)
{	//$key=getKey();
	$objCrypt=new CRYPTO();
	return $objCrypt->decrypt($str);		
}
function encryptStr($str)
{	//$key=getKey();
	$objCrypt=new CRYPTO();
	return $objCrypt->encrypt($str);		
}
?> 