...
if($_REQUEST['accept']!='')
{$obj_user->request_status = 1;
$obj_user->frnd_request_id = $_REQUEST['accept'];
$obj_user->insertUpdatebuyers_requestseller();

/////////
$accept_buyerid            = $_REQUEST['accept_buyerid'];
$obj_user->frnd_request_id ='';
$obj_user->request_status  = 1;
$obj_user->buyer_id        = $_SESSION['session_user_id'];
$obj_user->sellers_id      = $accept_buyerid;

$obj_user->insertUpdatebuyers_requestseller();

//////////
success_msg("Request of buyer is accepted successfully!!");
header("Location:compase_message.php");
}

if($_REQUEST['reject']!='')
{$obj_user->request_status = 2; // rejected
$obj_user->frnd_request_id=$_REQUEST['reject'];
$obj_user->insertUpdatebuyers_requestseller();
success_msg("Request of buyer is rejected successfully!!");
header("Location:compase_message.php");
}
...
$obj_user->sellers_id            = $_SESSION['session_user_id'];
$obj_user->request_status        = 0;

$resultDetails_of_users          = $obj_user->getDetails_ofbuyersrequest();
$num_result_users_details        = mysql_num_rows($resultDetails_of_users);
if($num_result_users_details >0)
{
 while($arr_fetch_resultDetails  = mysql_fetch_assoc($resultDetails_of_users))
  {
	$user_resultDetails[]    = $arr_fetch_resultDetails;
  }
}
//echo '<pre>';
//print_r($user_resultDetails);echo '</pre>';
$smarty->assign("user_resultDetails",$user_resultDetails);
$smarty->assign("num_value_users_details",$num_result_users_details);

if(isset($_POST['post']))
{

   	extract($_POST);
	//http://localhost/nethaat/view_messages.php?msg_id=37
