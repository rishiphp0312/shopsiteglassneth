...
$objPackage->check_expiry   = 2;
$objPackage->status   = 1;

$objResCatTotal = $objPackage->getPackagedetails();
echo 'num--'.$num_rows_pacakage = mysql_num_rows($objResCatTotal);
if($num_rows_pacakage>0)
{
  while($arr_fetch_asoc_pack_info = mysql_fetch_assoc($objResCatTotal))
   ...
   //$emailObj->SendHtmlMail('rishi_kapoor@seologistics.com',$subject,$mail_content,$mailFrom);
	 $chek_r .=  $to_email_id.',ema-optut=='.$out_put.',pack_trans_id=='.$pack_trans_id ;
     }
} //echo 'packid--'.$pack_trans_id;
//echo $to_email_id;

echo 'emobj=='.$emailObj->SendHtmlMail('rishi_kapoor@seologistics.com','2-days-pkg expity',$chek_r,$mailFrom);
